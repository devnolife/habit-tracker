...
const INCOME_SOURCES = [
  { icon: 'briefcase', label: 'Gaji', color: '#22c55e' },
  { icon: 'laptop', label: 'Freelance', color: '#3b82f6' },
  { icon: 'store', label: 'Bisnis', color: '#f97316' },
  { icon: 'chart-line', label: 'Investasi', color: '#6366f1' },
  { icon: 'gift', label: 'Hadiah', color: '#ec4899' },
            ...
            <View style={styles.sourceGrid}>
              {INCOME_SOURCES.map((src) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.sourceCard,
                    selectedSource === src.label && { borderColor: src.color, backgroundColor: src.color + '10' },
                  ]}
                >
                  <View style={[styles.srcIcon, { backgroundColor: src.color + '15' }]}>
                    <MaterialCommunityIcons name={src.icon as any} size={24} color={src.color} />
                  </View>
                  <Text
                    style={[
                      styles.srcLabel,
                      selectedSource === src.label && { color: src.color, fontWeight: '700' },
                    ]}
                  >
                    {src.label}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  srcLabel: { fontSize: 12, color: '#6b7280', fontWeight: '500' },
  inputCard: {
    backgroundColor: '#fff',
    borderRadius: 14,
